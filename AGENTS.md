# Claude Project Guidelines for go
...
Generate mocks before running tests and linting.
...
mock\_\*.go files are generated, if there are test errors, regenerate mocks.
...
`GITHUB_TOKEN` should be in the cloud environment (read access to private repos).
...
Go toolchain fallback, mock generation) relevant to Claude Code cloud sessions.
